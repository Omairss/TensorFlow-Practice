...
def rnn_data(data, time_steps, labels=False):
    ...
    for i in range(len(data) - time_steps):
        ...
        if labels:
            ...
            try:                    rnn_df.append(data.iloc[i + time_steps].as_matrix())
            except AttributeError:  rnn_df.append(data.iloc[i + time_steps])
...
def read_data(ticker, ohlc, time_steps, seperate):

    FILE_PATH = '../../../Finance_2/data/stocks/daily/'
    
    #files = [f for f in listdir(FILE_PATH) if isfile(join(FILE_PATH, f))]

    data      = pd.read_csv(FILE_PATH + ticker + '.csv')
    data      = data[[ohlc]]
